function [results, outPath] = run_experiment_var_crps(cfg, opts)
%RUN_EXPERIMENT_VAR_CRPS Function entrypoint for VaR+CRPS experiment.
%   [results, outPath] = run_experiment_var_crps(cfg, opts)
%
%   This is the function form of main/main_experiment_var_crps.m, suitable
%   for apps and programmatic runs (no clear/clc).

arguments
    cfg (1,1) struct = config()
    opts.doPlots (1,1) logical = false
    opts.nSamp (1,1) double {mustBePositive} = 800
end

rng(cfg.seed);

if cfg.horizon ~= 1
    warning('run_experiment_var_crps:HorizonNotImplemented', 'This experiment assumes cfg.horizon==1.');
end
...
results = struct();
results.meta = struct('timestamp', datetime('now'), 'entrypoint', 'run_experiment_var_crps', 'seed', cfg.seed);
results.cfg = cfg;
